...
void grammar::infer_epsilon(const token& non_terminator) 
{
	for (int i : productions_class[non_terminator])//遍历该非终结符对应的所有产生式
	{
		...
		const auto& j = current_production.right[0];//判断该产生式右侧第一个字符
		if (j == non_terminator)
		{
			continue;
		}
		if (find(terminators.begin(), terminators.end(), j) == terminators.end())//若为非终结符
		{
			if (epsilon_over[j] == false)
			{
				infer_epsilon(j);
				epsilon_over[j] = true;
			}	
			epsilon[non_terminator] = epsilon[non_terminator]|| epsilon[j];
		}
		else if (j == "ε")
